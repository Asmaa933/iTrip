...
public class HistoryMap extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        StartPoints= intent.getStringArrayListExtra(getApplicationContext().getString(R.string.start_points));
        EndPoints= intent.getStringArrayListExtra(getString(R.string.end_points));
            ...
            for(int i=0;i<StartPoints.size();i++)
            {
                ...
                for(Address address : addresses1){
                    if(address.hasLatitude() && address.hasLongitude()){
                        Loc1 = new LatLng(address.getLatitude(), address.getLongitude());
                    }
                }
                ...
                for(Address address : addresses2){
                    if(address.hasLatitude() && address.hasLongitude()){
                        Loc2 = new LatLng(address.getLatitude(), address.getLongitude());
                    }
                }
